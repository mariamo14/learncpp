...
- `break;` exits the loop immediately -- the loop ends right there, as if its condition had just become false.
- `continue;` skips the *rest of the current iteration* only, jumping straight to the next condition check (for a
  `while`) or the increment step (for a `for`) -- the loop itself keeps going.
