...
Understanding that `scores[i]` is really an address computation, not a special array feature, is what makes pointer
arithmetic, `malloc`'d arrays (next lesson), and eventually C-style strings all make sense as the same idea applied
in different places.
